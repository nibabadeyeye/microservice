package com.gpdi.springbootjjwt.exception;


import com.gpdi.springbootjjwt.data.ResponseData;
...
public class ExceptionHandlerAdvice {
    ...
    public ResponseData handle(BindException exception) {
        ...
        for (FieldError error : fieldErrors) {
           ...
           // System.out.println("xx" + JSON.toJSON(error));
        }
    ...
    public ResponseData RunTimehandle(Exception exception) {
        exception.printStackTrace();
        return new ResponseData(4011, "对不起，服务器内部错误！");
    ...
    public ResponseData handle(Exception exception) {
        exception.printStackTrace();
        return new ResponseData(401, "对不起，请先登录！");
